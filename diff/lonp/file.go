...
// FileDiff is context for calculating difference between a and b
...
// New is initializer of FileDiff
func New(a, b string) *FileDiff {
...
func (diff *FileDiff) FprintSes(w io.Writer) {
...
func (diff *FileDiff) snake(k, p, pp, offset int) int {
	r := 0
	if p > pp {
...
func (diff *FileDiff) recordSeq(epc []Point) {
	x, y := 1, 1
	px, py := 0, 0
